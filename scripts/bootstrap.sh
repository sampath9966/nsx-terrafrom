...
GITLAB_URL_ARG=""
# Where this generator came from, so the tree it writes can find updates.
SOURCE_URL="${NSX_BOOTSTRAP_SOURCE:-}"
...
while [ $# -gt 0 ]; do
	...
	--git-remote=*)
		...
		;;
	--source)
		[ $# -ge 2 ] || die "--source requires a git URL or path"
		SOURCE_URL="$2"
		shift 2
		;;
	--source=*)
...
if [ -f "$CONF" ]; then
	...
	# Running an OLDER generator over a NEWER tree silently downgrades every
	# generated file. Say so; do not guess what was meant.
	if [ -n "${SAVED_BOOTSTRAP_VERSION:-}" ] && [ "$SAVED_BOOTSTRAP_VERSION" != "$VERSION" ]; then
		older="$(printf '%s\n%s\n' "$SAVED_BOOTSTRAP_VERSION" "$VERSION" | sort -V | head -1)"
		if [ "$older" = "$VERSION" ]; then
			warn "this tree was generated by bootstrap $SAVED_BOOTSTRAP_VERSION and this script is $VERSION.
         Running an older generator over a newer tree downgrades its files.
         Use the newer generator, or scripts/update.sh, unless you mean to."
		else
			log "note: tree generated by $SAVED_BOOTSTRAP_VERSION; this script is $VERSION."
			log "      scripts/update.sh shows what changed before applying it."
			log ""
		fi
	fi
fi
...
# If this generator is itself a git checkout, its origin is where updates come
# from. Recorded in the generated tree so scripts/update.sh needs no arguments.
if [ -z "$SOURCE_URL" ]; then
	_self_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
	SOURCE_URL="$(git -C "$_self_dir" remote get-url origin 2>/dev/null || true)"
fi
...
.PHONY: update
update: ## Take a newer version of the generator. Shows the diff first.
	@scripts/update.sh

.PHONY: update-check
update-check: ## Is there a newer generator, and what changed? Changes nothing.
...
## Keeping up with the generator

A generated repository is a **copy, not a link**. Nothing phones home and
nothing updates itself — deliberate, because a tool that silently rewrites the
files controlling a production firewall would be worse than one that does
nothing. It does mean updating is something you run.

```bash
make update-check     # is there a newer generator, and what changed?
make update           # show the diff, then apply it if you agree
```

Each tree records where it came from and what built it, in
`.nsx-bootstrap.conf`:

```
SAVED_BOOTSTRAP_VERSION=1.1.0
SAVED_SOURCE=https://github.com/sampath9966/nsx-terraform.git
```

so `update.sh` needs no arguments. A tree that predates this takes the source
once and remembers it: `scripts/update.sh --from URL`.

**What an update may touch.** The generator owns `modules/`, `stacks/`,
`scripts/`, the schemas, CI and docs. You own `data/`, `inventory/` and `envs/`.
An update replaces the first and never the second — which is why it cannot break
your rules by editing them.

It can still change what your rules *mean*: a module that starts setting a field
differently changes the plan without changing your YAML. That is what the
changelog and the dry run are for, and why the last step is always a plan
against a non-production site.

**What it refuses:**

| | |
|---|---|
| A dirty working tree | `git diff` against your last commit is the only real undo |
| Applying unseen | every file that would be created or replaced is listed first |
| Going backwards | an older generator over a newer tree downgrades it — needs `--allow-downgrade` |
| Being quiet about a major bump | major means modules or schemas changed shape, and it says so |

Running an **older** `bootstrap.sh` directly over a newer tree is the one case
this cannot prevent, so it warns:

```
warning: this tree was generated by bootstrap 1.2.0 and this script is 1.1.0.
         Running an older generator over a newer tree downgrades its files.
```
...
make selftest     # the generator and validators still agree
make validate     # YOUR data still passes the new schemas
make ready        # anything new that now blocks production
```

`CHANGELOG.md` records what changed between versions and, more importantly,
which changes alter the meaning of data you already have.
...
write_file scripts/update.sh <<'SCAFFOLD_EOF'
#!/usr/bin/env bash
#
# Take a newer version of the generator into THIS repository.
#
#   scripts/update.sh --check     is there a newer version, and what changed?
#   scripts/update.sh             show the diff, then apply it if you agree
#   scripts/update.sh --from URL|PATH   use a specific source
#
# The problem this solves: a generated repository is a copy, not a link. Without
# this you would clone the generator somewhere else, notice it is newer, and
# hand-copy scripts/bootstrap.sh across — with no record of which version you
# were on and no idea what changed.
#
# What it will never do:
#   * touch data/, inventory/ or envs/ — your estate is yours
#   * run without showing you the diff first
#   * run on a dirty working tree, because git is the only real undo
#
# The generator owns modules/, stacks/, scripts/, schemas, CI and docs. You own
# the data. An update replaces the first set and leaves the second alone. That
# boundary is what makes updating safe; it is also why a module change can alter
# what your existing data MEANS, which is what the changelog is for.

set -uo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CONF="$REPO_ROOT/.nsx-bootstrap.conf"
CONFIRM_PHRASE="wipe everything & start fresh"
...
while [ $# -gt 0 ]; do
	...
	--from) SOURCE="${2:?--from needs a git URL or a path}"; shift 2 ;;
	--from=*) SOURCE="${1#*=}"; shift ;;
...
# --- where did this tree come from, and what version is it ----------------
...
if [ -f "$CONF" ]; then
	...
	current_version="${SAVED_BOOTSTRAP_VERSION:-unknown}"
	saved_source="${SAVED_SOURCE:-}"
fi
[ -n "$SOURCE" ] || SOURCE="$saved_source"

if [ -z "$SOURCE" ]; then
	die "no update source recorded, and none given.
       This tree predates version tracking, or .nsx-bootstrap.conf was removed.
       Point at the generator explicitly, once — it will be remembered:
         scripts/update.sh --from https://github.com/sampath9966/nsx-terraform.git
         scripts/update.sh --from /path/to/a/clone"
fi

step "this repository"
say "  root      $REPO_ROOT"
say "  version   $current_version"
say "  source    $SOURCE"

# --- fetch the candidate ---------------------------------------------------

WORK="$(mktemp -d "${TMPDIR:-/tmp}/nsx-update.XXXXXX")"
NEW="$WORK/src"

step "fetching"
if [ -d "$SOURCE" ]; then
	[ -f "$SOURCE/scripts/bootstrap.sh" ] ||
		die "$SOURCE has no scripts/bootstrap.sh — it is not a generator checkout."
	cp -r "$SOURCE" "$NEW" || die "could not copy from $SOURCE"
	say "  copied from $SOURCE"
else
	command -v git >/dev/null 2>&1 || die "git is required to fetch from a URL"
	git clone --depth 1 --quiet "$SOURCE" "$NEW" 2>/dev/null ||
		die "could not clone $SOURCE
       Check the URL and that you can reach it from here."
	say "  cloned $SOURCE"
fi

NEW_BOOTSTRAP="$NEW/scripts/bootstrap.sh"
[ -f "$NEW_BOOTSTRAP" ] || die "the source has no scripts/bootstrap.sh"

new_version="$(sed -n 's/^VERSION="\([^"]*\)".*/\1/p' "$NEW_BOOTSTRAP" | head -1)"
[ -n "$new_version" ] || new_version="unknown"
say "  available $new_version"

# --- compare ---------------------------------------------------------------

step "versions"
if [ "$current_version" = "$new_version" ]; then
	say "  already on $new_version — nothing to do."
	[ "$CHECK_ONLY" = 1 ] && exit 0
	say ""
	say "  (Files can still differ if this tree was edited. 'scripts/bootstrap.sh"
	say "   --dry-run' reports that; this script only handles version changes.)"
	exit 0
fi

if [ "$current_version" = unknown ]; then
	say "  this tree does not record a version; treating $new_version as newer."
else
	older="$(printf '%s\n%s\n' "$current_version" "$new_version" | sort -V | head -1)"
	if [ "$older" = "$new_version" ] && [ "$current_version" != "$new_version" ]; then
		if [ "$ALLOW_DOWNGRADE" = 1 ]; then
			say "  $new_version is OLDER than $current_version — downgrading as asked."
		else
			die "the source is $new_version, older than this tree's $current_version.
       Updating would replace newer generated files with older ones.
       Pass --allow-downgrade if that is genuinely what you want."
		fi
	fi

	say "  $current_version -> $new_version"

	cur_major="${current_version%%.*}"
	new_major="${new_version%%.*}"
	if [ "$cur_major" != "$new_major" ]; then
		say ""
		say "  *** MAJOR VERSION CHANGE: $current_version -> $new_version ***"
		say "  Major means the generated modules or the data schemas changed shape."
		say "  Read the changelog below in full, and plan a terraform plan against"
		say "  a non-production site before applying anything to the estate."
	fi
fi
...
if [ -f "$NEW/CHANGELOG.md" ]; then
	# Everything above the heading for the version we are on.
	awk -v cur="$current_version" '
		...
		{ print "  " $0 }
	' "$NEW/CHANGELOG.md" | head -60
	say ""
	say "  (full text: $NEW/CHANGELOG.md — copied into this tree when you apply)"
else
	say "  the source has no CHANGELOG.md; nothing to show."
fi

[ "$CHECK_ONLY" = 1 ] && {
	say ""
	say "--check: nothing was changed. Run without it to see the diff and apply."
	exit 0
}
...
if git -C "$REPO_ROOT" rev-parse --git-dir >/dev/null 2>&1; then
	if [ -n "$(git -C "$REPO_ROOT" status --porcelain=v1 2>/dev/null)" ]; then
		if [ "$ALLOW_DIRTY" = 1 ]; then
			say "  dirty, continuing as asked — you are giving up your undo."
		else
			die "the working tree has uncommitted changes.
       Commit or stash first: an update overwrites generated files, and the
       diff against your last commit is the only way to see what it did.
       Pass --allow-dirty to override, knowing that."
		fi
	else
		say "  clean — the update will be a reviewable diff"
	fi
else
	say "  not a git repository, so there is no undo. Consider:"
	say "    scripts/enable-gitops.sh"
fi

# --- show the diff ---------------------------------------------------------

step "what would change"
# --force as well as --dry-run: without it the preview reports files as "kept"
# and counts nothing, which is the opposite of what an update is about to do.
# --dry-run never prompts and never writes, so this is safe.
preview="$("$NEW_BOOTSTRAP" --dir "$REPO_ROOT" --dry-run --force \
	--confirm "$CONFIRM_PHRASE" --no-interactive 2>&1)"

printf '%s\n' "$preview" | grep -E 'would (create|overwrite)' | sed 's/^/  /' | head -40
n_new="$(printf '%s\n' "$preview" | grep -c 'would create' || true)"
n_over="$(printf '%s\n' "$preview" | grep -c 'would overwrite' || true)"
n_kept="$(printf '%s\n' "$preview" | grep -c 'would keep' || true)"

say ""
say "  $n_new new, $n_over replaced, $n_kept protected."
say "  data/, inventory/ and envs/ are never touched by an update."
if [ "$n_new" = 0 ] && [ "$n_over" = 0 ]; then
	say ""
	say "  Nothing to change: the generated files already match $new_version."
	say "  Only the recorded version differs."
fi
...
if [ "$ASSUME_YES" != 1 ]; then
	step "apply?"
	if ! { true </dev/tty; } 2>/dev/null; then
		die "no terminal to confirm on. Re-run with --yes in a pipeline."
	fi
	printf 'Apply this update? [y/N]: ' >&2
	IFS= read -r reply </dev/tty || reply=""
...
step "applying"
if "$NEW_BOOTSTRAP" --dir "$REPO_ROOT" --force --confirm "$CONFIRM_PHRASE" --no-interactive; then
	say ""
	say "updated $current_version -> $new_version"
else
	die "the update did not complete. Your tree is a git working copy —
       'git diff' shows exactly how far it got, and 'git checkout -- .' undoes it."
fi

# Carry the changelog across so the next person can see the history.
[ -f "$NEW/CHANGELOG.md" ] && cp "$NEW/CHANGELOG.md" "$REPO_ROOT/CHANGELOG.md" 2>/dev/null

cat <<NEXT

Next:

  git diff                  what the update actually changed
  make selftest             the generator and validators still agree
  make validate             YOUR data still passes the new schemas
  make ready                anything new that now blocks production

Then plan against a NON-PRODUCTION site before anything else. An update can
change what your existing data means, even though it never edited your data.

NEXT
SCAFFOLD_EOF
mark_executable scripts/update.sh
...
# The changelog lives at the root, not under docs/, because that is where people
# look for it. Same carry-across: too long to embed, must travel with the script.
if [ -f "$SELF_DIR/../CHANGELOG.md" ] &&
	[ "$SELF_DIR/../CHANGELOG.md" != "$ROOT/CHANGELOG.md" ]; then
	write_file CHANGELOG.md <"$SELF_DIR/../CHANGELOG.md"
fi
...
if [ "$DRY_RUN" != 1 ]; then
	{
		...
		# Provenance. Without these, scripts/update.sh cannot tell whether a
		# newer generator exists, and nobody can tell what produced this tree.
		printf 'SAVED_BOOTSTRAP_VERSION=%s\n' "$VERSION"
