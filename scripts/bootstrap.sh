#!/usr/bin/env bash
#
# bootstrap.sh — scaffold the NSX/VCF Terraform repository described in CLAUDE.md.
#
...
# ---------------------------------------------------------------------------
# 1. Directory structure (CLAUDE.md section 4, plus data/network and
...
# posture of the estate — it never lands in git. See CLAUDE.md section 9.
.terraform/
*.tfstate
*.tfstate.*
...
# one, document it in CLAUDE.md section 16 in the same commit.
#
# Usage:
#   make help
#   make validate
...
# change (CLAUDE.md section 11), not a routine one.
#
...
scopes:
  ...
  quarantine:
    description: >
      ...
      Emergency-category mechanism in CLAUDE.md section 11.
...
write_file data/schema/platform.schema.json <<'SCAFFOLD_EOF'
{
  ...
  "title": "Per-site platform objects (RESTRICTED — CLAUDE.md section 11)",
  "type": "object",
  "required": ["site"],
  "additionalProperties": false,
  "properties": {
...
# consumes them (CLAUDE.md section 7).
...
variable "tier0s" {
  description = "Map of tier-0 gateway definitions keyed by stable logical name. RESTRICTED — see CLAUDE.md section 11."
  type        = any
  default     = {}
}
SCAFFOLD_EOF
...
#
# OPEN DECISION (CLAUDE.md section 14.1): the backend type is the owner's call
...
Every change here is **restricted**. An AI assistant may prepare one; it may
never apply one.
SCAFFOLD_EOF
...
#   terraform init -backend-config=envs/<site>.backend.hcl
#
# The backend type is an open decision (CLAUDE.md section 14.1). Whatever is
...
# below and decide the state backend (CLAUDE.md section 14).
name: plan
...
what still needs a human decision (CLAUDE.md section 14)
